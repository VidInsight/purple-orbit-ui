...
export const WorkspaceProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    // Listen for storage changes (e.g., when workspace is selected in another tab/component)
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'current_workspace' || e.key === 'workspaces') {
        loadWorkspaces();
      }
    };
    ...
    window.addEventListener('workspaceStorageChange', handleCustomStorageChange);

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('workspaceStorageChange', handleCustomStorageChange);
    };
  }, []);
